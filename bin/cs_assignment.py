...
print "DEBUG: CHAINS_BIN_DIR=", CHAINS_BIN_DIR
print "DEBUG: CHAINS_LIB_DIR=", CHAINS_LIB_DIR
...
def cmdlineparse():
    ...
    parser.add_argument("-rstart", dest="FIRST_RESIDUE_NUMBER", required=True, default=1, type=int, 
                        help="the number of the first residue in the protein sequence (default: 1)", metavar="<first residue number>")
...
print "DEBUG: Tindex_CSlist_dict=", Tindex_CSlist_dict
    ...
    for residue in residue2Tindex_dict.keys():
        if residue2Tindex_dict[residue] == None:
            ...
            try:
                ...
                print "ERROR: residue ", residue, " was patched but has no NOESY peaks!!!"
                sys.exit(1)
...
if args.OUT_fname:
    ...
    xeasy_fout_name = args.ABSOLUTE_MATCHES_FILE+".xeasy"
...
for res_index, residue in enumerate(residue_list):
    ...
    for TOCSY_H_resonance,TOCSY_C_resonance,TOCSY_N_resonance,TOCSY_HN_resonance in residue_CSlist_dict[residue]:
        ...
        if args.USE_2D_HISTOGRAMS == True:
            valid_matches_list = get_all_assignments_from_H_C_resonpair_2Dhist(aa_type, float(TOCSY_H_resonance), float(TOCSY_C_resonance), Num_of_TOCSY_resonances,
                                                                               aa_CHpair_binProbabilityList_multidict, aa_carbon_binDensityList_multidict,
                                                                               useonlyCarbon=False)
        else:
