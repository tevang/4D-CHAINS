...
def cmdlineparse():
    ...
    parser.add_argument("-absfile", dest="ABSOLUTE_MATCHES_FILE", required=False,
                        help="file with absolute matches from previous run", metavar="<absolute matches file>")
    ...
    parser.add_argument("-atocsy", dest="TOCSY_fname", required=True,
                        help="4D TOCSY (HCTOCSYNH) file with atom assignments", metavar="<4D TOCSY input file with atom assignments>")
    parser.add_argument("-noesy", dest="NOESY_fname", required=True,
                        help="the 4D NOESY file (*num.list) produced by 4D_assignment_parallel.py", metavar="<4D NOESY input file>")
...
TOCSY_residue_assignments_dict, TOCSY_residue_NHresonances_dict, i_to_iminus1_dict, patched_residues_list, TOCSY_residue_peak_intensity_multidict = read_assigned_TOCSY_file(args.TOCSY_fname, "TOCSY", absolute_RIGmatches_alignment_list, absolute_matches_alignment_list)
...
for k,v in TOCSY_residue_assignments_dict.items():
    print k, "-->", v


NOESY_residue_assignments_dict, NOESY_residue_NHresonances_dict, original_NOESY_peaks_dict, NOESY_residue_peak_intensity_multidict = read_assigned_TOCSY_file(args.NOESY_fname, 'NOESY', absolute_RIGmatches_alignment_list, absolute_matches_alignment_list, i_to_iminus1_dict)
...
print "DEBUG: NOESY_residue_peak_intensity_multidict=", NOESY_residue_peak_intensity_multidict
for k,v in NOESY_residue_assignments_dict.items():
    print k, "-->", v

matched_NOESY_residue_assignments_dict, residue_unmatched_TOCSY_peaks_dict = match_TOCSY_to_NOESY_lines(TOCSY_residue_assignments_dict,
                                                        NOESY_residue_assignments_dict, protein_alignment_list, absolute_matches_alignment_list, args)
...
revordered_residue_keys = [(k[0], int(k[1:])) for k in TOCSY_residue_assignments_dict.keys()]
...
for i_aa_resid in selected_revordered_residue_keys:    # both the keys and the C,H assignments correspond to residue i-1
    ...
    for I_THRESHOLD in reversed(range(1, istart+1, 1)):
        ...
        if i_residue in TOCSY_residue_assignments_dict.keys() and len(TOCSY_residue_assignments_dict[i_residue]) > 0:   # if no TOCSY assignments have been made for residue i, skip NOESY assignment of residue i
            ...
            if len(missing_carbons_list) != 0:   # IF THERE ARE MISSING CARBONS IN TOCSY, TRY TO FIND THEM IN NOESY (EXCLUDE THE ASSIGNED OR MATCHED NOESY PEAKS)
                ...
                for NOESY_peak in unassigned_NOESY_peaks_list:  # make C-H type predictions for each unassigned NOESY peak
                    ...
                    all_possible_assignments_list.extend( get_probabilities_from_H_C_resonpair_2Dhist(i_residue, NOESY_Hreson,
                                NOESY_Creson, missing_carbons_list, partly_assigned_carbons_list,
                                args, aa_CHpair_binProbabilityList_multidict, aa_carbon_binDensityList_multidict,
                                NOESY_residue_peak_intensity_multidict, matched_i_to_iplus1_peaks_multidict, aa2pthres_dict,
                                aa2ithres_dict, protein_alignment_list, absolute_matches_alignment_list, aa_hydrogen_binDensityList_multidict,
                                iteration=1, I_THRESHOLD=I_THRESHOLD)
                                                         )
                ...
                if len(all_possible_assignments_list) > 0:  # IF WE HAVE FOUND POSSIBLE NOESY PEAK ASSIGNMENTS OF THE MISSING CARBONS
                    written_nucleiNames_list = select_best_NOESY_peak_combination(i_residue, all_possible_assignments_list, partly_assigned_carbons_list,
                                                                                  atom_index, xeasy_fout, sparky_fout, protein_alignment_list,
                                                                                absolute_matches_alignment_list, args, NOESY_residue_NHresonances_dict,
                                                                                    matched_NOESY_residue_assignments_dict, TOCSY_assigned_residue_list, 
                                                                                    iter1_allowed_atoms_dict, iteration=1)
                residues_with_written_TOCSY_peaks.append(i_residue)
                if write_TOCSY_NOESY_matched_peaks(i_residue, atom_index, matched_NOESY_residue_assignments_dict, xeasy_fout, sparky_fout, 
                                    protein_alignment_list, absolute_matches_alignment_list, args, TOCSY_assigned_residue_list,
                ...
                for TOCSY_assignment in TOCSY_residue_assignments_dict[i_residue]:
                    ...
                    if i_residue in matched_NOESY_residue_assignments_dict.keys():  # first check if this residue has NOESY peaks
                        ...
                        if len(matched_NOESY_peak) == 1:    # if this TOCSY peak has been matched with a NOESY peak, use the NOESY resonances (more accurate)
                            ...
                            sys.exit(1)
                    all_possible_assignments_list.append([aa_type, Cname, Hname, 10000000.0, Hreson, Creson, spectrum_type])  # set the probability to 1 cause we don't know it
...
write_unmatched_TOCSY_peaks(matched_NOESY_residue_assignments_dict, TOCSY_residue_assignments_dict, atom_index, residues_with_written_NH_list,
                                xeasy_fout, sparky_fout, protein_alignment_list, absolute_matches_alignment_list, args, 
    ...
    for resid in proofread_clean_resid_assignments_dict.keys():
        ...
        for assignment in proofread_clean_resid_assignments_dict[resid]:
            ...
            if aa_type in aatype_carbon_nongeminalHname_multidict.keys() and nucleus in aatype_carbon_nongeminalHname_multidict[aa_type].keys():
                ...
                comment = ""
                if spectrum_type in ['NOESY', 'NOESY (C-term hanging residue)', 'NOESY (flanked residue)', 'NOESY + TOCSY-NOESY', 'TOCSY-NOESY', 'TOCSY-NOESY + NOESY', 'TOCSY (unmatched) + TOCSY-NOESY', 'TOCSY-NOESY + TOCSY (unmatched)']:    # if this resonance comes only from NOESY
                    user_assignment = get_Carbon_resonance(resid, nucleus, 'NOESY', user_NOESY_resid_assignments_dict, user_TOCSY_resid_assignments_dict, aa_equivalentCarbons_dict,
                                                       protein_alignment_list, absolute_matches_alignment_list, args, get_assignment=True)
                    if user_assignment == False:    # if no user assignment was made for this Carbon, add no extra comment
                        ...
                        user_assignment = get_Carbon_resonance(resid, nucleus, 'TOCSY', user_NOESY_resid_assignments_dict, user_TOCSY_resid_assignments_dict, aa_equivalentCarbons_dict,
                                                       protein_alignment_list, absolute_matches_alignment_list, args, get_assignment=True)
                ...
                if spectrum_type in ['NOESY', 'NOESY (C-term hanging residue)', 'NOESY (flanked residue)', 'NOESY + TOCSY-NOESY', 'TOCSY-NOESY', 'TOCSY-NOESY + NOESY', 'TOCSY (unmatched) + TOCSY-NOESY', 'TOCSY-NOESY + TOCSY (unmatched)']:    # if this resonance comes only from NOESY
                    user_assignment = get_Carbon_resonance(resid, nucleus, 'NOESY', user_NOESY_resid_assignments_dict, user_TOCSY_resid_assignments_dict, aa_equivalentCarbons_dict,
                                                       protein_alignment_list, absolute_matches_alignment_list, args)
                    if user_assignment == False:    # if no user assignment was made for this Carbon, add no extra comment
                        print "WARNING: no user assignment made in NOESY for resid", resid, "and Carbon", nucleus,". Checking TOCSY..."
                        user_assignment = get_Carbon_resonance(resid, nucleus, 'TOCSY', user_NOESY_resid_assignments_dict, user_TOCSY_resid_assignments_dict, aa_equivalentCarbons_dict,
                                                       protein_alignment_list, absolute_matches_alignment_list, args)
    ...
    if i_residue in TOCSY_residue_assignments_dict.keys() and len(TOCSY_residue_assignments_dict[i_residue]) > 0:   # if no TOCSY assignments have been made for residue i, skip NOESY assignment of residue i
        ...
        if len(missing_carbons_list) != 0:   # IF THERE ARE MISSING CARBONS IN TOCSY, TRY TO FIND THEM IN NOESY (EXCLUDE THE ASSIGNED OR MATCHED NOESY PEAKS)
            ...
            for NOESY_peak in unassigned_NOESY_peaks_list:  # make C-H type predictions for each unassigned NOESY peak
                NOESY_Creson = NOESY_peak[2]
                NOESY_Hreson = NOESY_peak[4]
                all_possible_assignments_list.extend( get_probabilities_from_H_C_resonpair_2Dhist(i_residue, NOESY_Hreson, NOESY_Creson,
                                                    missing_carbons_list, partly_assigned_carbons_list, args, aa_CHpair_binProbabilityList_multidict,
                                                    aa_carbon_binDensityList_multidict, NOESY_residue_peak_intensity_multidict,
                                                    matched_i_to_iplus1_peaks_multidict, aa2pthres_dict, aa2ithres_dict, protein_alignment_list,
                                                    absolute_matches_alignment_list, aa_hydrogen_binDensityList_multidict, iteration=2)
                                                    )
            [assignment.append('NOESY') for assignment in all_possible_assignments_list]    # we don't need to created a new list, the original all_possible_assignments_list will be updated
            print "DEBUG: i_residue=", i_residue, "all_possible_assignments_list=", all_possible_assignments_list
            ...
            if len(all_possible_assignments_list) > 0:  # IF WE HAVE FOUND POSSIBLE NOESY PEAK ASSIGNMENTS OF THE MISSING CARBONS
                written_nucleiNames_list.extend(select_best_NOESY_peak_combination(i_residue, all_possible_assignments_list, partly_assigned_carbons_list,
                                                                                   atom_index, xeasy_fout, sparky_fout, protein_alignment_list,
                                                                                   absolute_matches_alignment_list, args, NOESY_residue_NHresonances_dict,
                                                                                    matched_NOESY_residue_assignments_dict, TOCSY_assigned_residue_list, 
                                                                                    iter1_allowed_atoms_dict, iteration=2))
...
write_flanked_residues(absolute_matches_alignment_list, matched_NOESY_residue_assignments_dict, TOCSY_residue_assignments_dict, 
                           ...
                           protein_alignment_list, args, aa_CHpair_binProbabilityList_multidict, aa_carbon_binDensityList_multidict,
                           NOESY_residue_peak_intensity_multidict, aa2pthres_dict, aa2ithres_dict, aa_hydrogen_binDensityList_multidict,
                           checked_residues_set, matched_i_to_iplus1_peaks_multidict, 
                           NOESY_residue_NHresonances_dict, residues_with_written_NH_list, residues_with_written_prevAssigned_peaks, 
                           revordered_residue_keys, iter1_allowed_atoms_dict, iteration=2)
        ...
        if len(missing_carbons_list) != 0:   # IF THERE ARE MISSING CARBONS IN ITERATION 1, TRY TO FIND THEM IN NOESY (EXCLUDE THE ASSIGNED OR MATCHED NOESY PEAKS)
            ...
            for NOESY_peak in unassigned_NOESY_peaks_list:  # make C-H type predictions for each unassigned NOESY peak
                ...
                all_possible_assignments_list.extend( get_probabilities_from_H_C_resonpair_2Dhist(i_residue, NOESY_Hreson, NOESY_Creson,
                                                      ...
                                                      aa_carbon_binDensityList_multidict, NOESY_residue_peak_intensity_multidict,
                                                      matched_i_to_iplus1_peaks_multidict, aa2pthres_dict, aa2ithres_dict, protein_alignment_list,
                                                      absolute_matches_alignment_list, aa_hydrogen_binDensityList_multidict, iteration=3, I_THRESHOLD=I_THRESHOLD)
                                                    )
            [assignment.append('NOESY') for assignment in all_possible_assignments_list]    # we don't need to created a new list, the original all_possible_assignments_list will be updated
            print "DEBUG: i_residue=", i_residue, "all_possible_assignments_list=", all_possible_assignments_list
            ...
            if len(all_possible_assignments_list) > 0:  # IF WE HAVE FOUND POSSIBLE NOESY PEAK ASSIGNMENTS OF THE MISSING CARBONS
                written_nucleiNames_list.extend(select_best_NOESY_peak_combination(i_residue, all_possible_assignments_list, partly_assigned_carbons_list,
                                                                                   ...
                                                                                   absolute_matches_alignment_list, args, NOESY_residue_NHresonances_dict,
                                                                                   matched_NOESY_residue_assignments_dict, TOCSY_assigned_residue_list, 
                                                                                   iter1_allowed_atoms_dict, iteration=3))
            residues_with_written_prevAssigned_peaks.append(i_residue)
            write_peaks_assigned_in_previous_iteration(i_residue, clean_resid_assignments_dict, atom_index, xeasy_fout, 
                                               NOESY_residue_NHresonances_dict, residues_with_written_NH_list)
        elif len(missing_carbons_list) == 0 and len(partly_assigned_carbons_list) == 0: # NOTHING TO DO FOR THIS RESIDUE
    ...
    if len(missing_carbons_list) != 0:   # IF THERE ARE MISSING CARBONS IN ITERATION 1, TRY TO FIND THEM IN NOESY (EXCLUDE THE ASSIGNED OR MATCHED NOESY PEAKS)
        ...
        for NOESY_peak in unassigned_NOESY_peaks_list:  # make C-H type predictions for each unassigned NOESY peak
            ...
            all_possible_assignments_list.extend( get_probabilities_from_H_C_resonpair_2Dhist(i_residue, NOESY_Hreson, NOESY_Creson,
                                                  ...
                                                  aa_carbon_binDensityList_multidict, NOESY_residue_peak_intensity_multidict,
                                                  matched_i_to_iplus1_peaks_multidict, aa2pthres_dict, aa2ithres_dict, protein_alignment_list,
                                                  absolute_matches_alignment_list, aa_hydrogen_binDensityList_multidict, iteration=4)
                                                )
        [assignment.append('NOESY') for assignment in all_possible_assignments_list]    # we don't need to created a new list, the original all_possible_assignments_list will be updated
        print "DEBUG: i_residue=", i_residue, "all_possible_assignments_list=", all_possible_assignments_list
        ...
        if len(all_possible_assignments_list) > 0:  # IF WE HAVE FOUND POSSIBLE NOESY PEAK ASSIGNMENTS OF THE MISSING CARBONS
            written_nucleiNames_list.extend(select_best_NOESY_peak_combination(i_residue, all_possible_assignments_list, partly_assigned_carbons_list,
                                                            atom_index, xeasy_fout, sparky_fout, protein_alignment_list, absolute_matches_alignment_list, args, NOESY_residue_NHresonances_dict,
        ...
        if len(all_possible_assignments_list) > 0:  # IF WE HAVE FOUND POSSIBLE NOESY PEAK ASSIGNMENTS OF THE MISSING CARBONS
            written_nucleiNames_list.extend(select_best_NOESY_peak_combination(i_residue, all_possible_assignments_list, partly_assigned_carbons_list,
                                                                               ...
                                                                               absolute_matches_alignment_list, args, NOESY_residue_NHresonances_dict,
                                                                               matched_NOESY_residue_assignments_dict, TOCSY_assigned_residue_list, 
                                                                               iter1_allowed_atoms_dict, iteration=4))
    ...
    if len(missing_carbons_list) != 0:   # IF THERE ARE MISSING CARBONS IN ITERATION 1 & 2, TRY TO FIND THEM IN NOESY (EXCLUDE THE ASSIGNED OR MATCHED NOESY PEAKS)
        ...
        for NOESY_peak in unassigned_NOESY_peaks_list:  # make C-H type predictions for each unassigned NOESY peak
            ...
            all_possible_assignments_list.extend( get_probabilities_from_H_C_resonpair_2Dhist(i_residue, NOESY_Hreson, NOESY_Creson,
                                                  ...
                                                  aa_carbon_binDensityList_multidict, NOESY_residue_peak_intensity_multidict,
                                                  matched_i_to_iplus1_peaks_multidict, aa2pthres_dict, aa2ithres_dict, protein_alignment_list,
                                                  absolute_matches_alignment_list, aa_hydrogen_binDensityList_multidict, iteration=5)
                                                )
        [assignment.append('NOESY') for assignment in all_possible_assignments_list]    # we don't need to created a new list, the original all_possible_assignments_list will be updated
        print "DEBUG: i_residue=", i_residue, "all_possible_assignments_list=", all_possible_assignments_list
        written_nucleiNames_list = []
        ...
        if len(all_possible_assignments_list) > 0:  # IF WE HAVE FOUND POSSIBLE NOESY PEAK ASSIGNMENTS OF THE MISSING CARBONS
            written_nucleiNames_list.extend(select_best_NOESY_peak_combination(i_residue, all_possible_assignments_list, partly_assigned_carbons_list,
                                                                               ...
                                                                               absolute_matches_alignment_list, args, NOESY_residue_NHresonances_dict,
                                                                               matched_NOESY_residue_assignments_dict, TOCSY_assigned_residue_list, 
                                                                               iter1_allowed_atoms_dict, iteration=5))
        residues_with_written_prevAssigned_peaks.append(i_residue)
        write_peaks_assigned_in_previous_iteration(i_residue, clean_resid_assignments_dict, atom_index, xeasy_fout, 
                                               NOESY_residue_NHresonances_dict, residues_with_written_NH_list)
    elif len(missing_carbons_list) == 0 and len(partly_assigned_carbons_list) == 0: # NOTHING TO DO FOR THIS RESIDUE
...
annotate_NOESY_file(args.NOESY_fname, absolute_RIGmatches_alignment_list, absolute_matches_alignment_list,
                    matched_NOESY_residue_assignments_dict, NOESY_residue_peak_intensity_multidict, out_fname="4DNOESY_assignedall.sparky")
