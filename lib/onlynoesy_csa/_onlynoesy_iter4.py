...
def onlynoesy_csa_iter4(self, iter3_pkl=None):

    if iter3_pkl:
        # LOAD THE NECESSARY DATA FROM ITERATION 3
        ...
        self.user_VAL_LEU_TOCSY_resid_assignments_dict = load_pickle(iter3_pkl)

    print("DEBUG: ENTERING ITERATION 4 self.matched_HCNH_residue_assignments_dict=", self.matched_HCNH_residue_assignments_dict)
    print("DEBUG: ENTERING ITERATION 4 self.clean_resid_assignments_dict=", self.clean_resid_assignments_dict)

    xeasy_fout = open(self.args.NHmap_FILE+".HCNH.iter4.xeasy", 'w')
    sparky_fout = open(self.args.NHmap_FILE+".HCNH.iter4.sparky", 'w')
    atom_index = 1
    ...
    for i_aa_resid in self.revordered_residue_keys_iter3:    # both the keys and the C,H assignments correspond to residue i-1
        ...
        print("ITERATION 4: Matching residue i ", i_residue," TOCSY PEAKS to residue's i HCNH peaks...")
        ...
        if len(missing_carbons_list) != 0:   # IF THERE ARE MISSING CARBONS IN ITERATION 1, TRY TO FIND THEM IN HCNH (EXCLUDE THE ASSIGNED OR MATCHED HCNH PEAKS)
            print("Residue ", i_residue, " was not completely assigned in iteration 1. Using HCNH to assigned resonances to the rest Carbon types.")
            ...
            for HCNH_peak in unassigned_HCNH_peaks_list:  # make C-H type predictions for each unassigned HCNH peak
                ...
                # OLD                                                            self.aa2pthres_dict.aa_hydrogen_binDensityList_mdict, iteration=4))
            ...
            if i_resid in list(self.clean_resid_assignments_dict.keys()):  # if this is a C-term residue that is analyzed for the first time in iter4
    ...
    write_NH_of_residues_without_CH(self.ali.absolute_matches_alignment_list,
                                    self.HCNH_residue_NHresonances_dict,
                                    {}, # it was TOCSY_residue_NHresonances_dict before
                                    self.residues_with_written_NH_list,
                                    atom_index,
                                    xeasy_fout,
                                    sparky_fout,
    ...
    self.clean_resid_assignments_dict = clean_xeasy_file(self.args.NHmap_FILE+".HCNH.iter4.xeasy")  # dict with all the assignments from each residue from iteration 1
    ...
    print("DEBUG: ITERATION 4 point 1 self.clean_resid_assignments_dict=", self.clean_resid_assignments_dict)

    ## WRITE THE CLEAN XEASY FILE
    xeasy_out = open(self.args.NHmap_FILE+".HCNH.iter4.xeasy", 'w')
    ...
    self.clean_resid_assignments_dict = clean_xeasy_file(self.args.NHmap_FILE+".HCNH.iter4.xeasy")
    # sort the peak assignments of each resid
    for resid, peak_list in list(self.clean_resid_assignments_dict.items()):
        peak_list.sort(key=itemgetter(3))
        self.clean_resid_assignments_dict[resid] = peak_list

    print("DEBUG: ITERATION 4 point 2 self.clean_resid_assignments_dict=", self.clean_resid_assignments_dict)

    ## WRITE THE CLEAN XEASY FILE
    xeasy_out = open(self.args.NHmap_FILE+".HCNH.cleaned.iter4.xeasy", 'w')
    atom_index = 0  # reinitialize atom index
    for resid in list(self.clean_resid_assignments_dict.keys()):
        for peak in self.clean_resid_assignments_dict[resid]:
            atom_index += 1
    ...
    if self.args.user_TOCSY_FILE and self.args.user_HCNH_FILE:
        self.proofread_iter34(iteration="4")

    # Save the necessary objects for the next ITERATION
    save_pickle("iter4.pkl",
                self.matched_HCNH_residue_assignments_dict,
                self.checked_residues_set,
                self.TOCSY_assigned_residue_list,
                self.residues_with_written_NH_list,
...
# def proofread_iter4_short(self):
#     """
#     same as proofread_iter3()
#     :param self:
#     :return:
#     """
#
...
#     xeasy_out = open(self.args.NHmap_FILE + ".HCNH.proofread.iter4.xeasy", 'w')
#     atom_index = 0  # reinitialize atom index
#     for resid in list(self.proofread_clean_resid_assignments_dict.keys()):
