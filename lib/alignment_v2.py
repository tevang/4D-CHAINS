...
class Alignment():
    ...
    def __init__(self, ABSOLUTE_MATCHES_FILE, FIRST_RESIDUE_NUMBER):
        self.ABSOLUTE_MATCHES_FILE, self.FIRST_RESIDUE_NUMBER = ABSOLUTE_MATCHES_FILE, FIRST_RESIDUE_NUMBER
        self.absolute_AAIGmatches_alignment_list = []
        self.protein_alignment_list = []
        self.absolute_matches_alignment_list = []
                ...
                if line[0:53] == "CONSENSUS CONFIDENCE TOP-SCORED AND ABSOLUTE MATCHES:" or \
                    ...
                    try:
                        while True:
                            ...
                            # OLD Python2.7: protein_alignment_list.extend(line.replace('\xe2\x94\x82', '').split())
                            protein_alignment_list.extend(line.replace('│', '').split())
                            line = next(f)
                            line = next(f)
                            # print "DEBUG: Matches:", line
                            # OLD Python2.7: absolute_matches_alignment_list.extend(line.replace('\xe2\x94\x82', '').replace('*', '-').split())
    ...
    def create_absolute_AAIGmatches_alignment(self):
        """
        Method to READ FILE WITH ABSOLUTE MATCHES
        :param ABSOLUTE_MATCHES_FILE:
        :populate: self.absolute_AAIGmatches_alignment_list:     contains the absolutely matched root index groups (RIG),
    ...
    def create_absolute_matches_alignment(self, spectrum_combo="TOCSY-HCNH"):
        """
        ...
        self.absolute_matches_alignment_list = ['-'] * len(
            self.absolute_AAIGmatches_alignment_list)  # contains the absolutely matched residue names,
...
def get_alignments_with_unique_AAIG(consensus_overlappingChains_alignment_list, max_peptide_length):
    """
        FUNCTION to break long consensus sequence alignments with multiple occurrences of one or more AAIGs
...
def get_alignment_with_absolute_matches(consensus_sequence_alignments_set, args):
    ...
    if args.ABSOLUTE_MATCHES_FILE:
        ...
        if args.SPECTRUM_COMBO == "HCNH-HCNH" and raw_restraints_alignment[0] == 'N/A':
            raw_restraints_alignment = raw_restraints_alignment[1:]     # remove 'N/A' at the beginning
        restraints_alignment = ['*' if x == '-' else x for x in raw_restraints_alignment]   # convert '-' to '*'
    ...
    ## CLEAN THE ABSOLUTE MATCHES ALIGNMENT FROM DUPLICATE AAIGs
    ...
    # NOW CLEAN THE GROUPS OF DUPLICATE AAIGs FROM OVERHANGING ENDS AND KEEP ONLY THOSE THAT CONTAIN ONE NON-OVERHANGING AAIG
        ...
        for index in index_group:
            ...
            for chain_alignment_list in consensus_sequence_alignments_set:
                if chain_alignment_list[index] == TAAIG:   # just found a consensus sequence alignment with the current TAAIG
                    if index == 0:  # in case it is N-term or C-term it's still an overhanging end ????
    ...
    chain_list = []
    if args.SPECTRUM_COMBO == "TOCSY-HCNH":
        chain_list.append(' N/A ')
    for word in absolute_matches_alignment:
        if word == '*':
            chain_list.append('*')
        else:
...
def create_consensus_alignemnt(peptide_identity_dict,
    ...
    """
    if SPECTRUM_COMBO == "HCNH-HCNH":
        SHIFT = 0
    elif SPECTRUM_COMBO == "TOCSY-HCNH":
        SHIFT = -1
    ...
    for start_column in range(len(consensus_chain_alignment_list[0].split())-2):  # the minimum peptide length is 2 so end at position -2
        ...
        index = 0
        for chain_alignment_string, peptide_name, peptide_line, TAAIGScoreTuple_list in zip(consensus_chain_alignment_list,
                                                                                            consensus_peptideName_list,
                                                                                            consensus_peptide_alignment_list,
                                                                                            consensus_TAAIGScore_alignment_list):
...
def write_consensus_alignment(ordered_consensus_peptide_alignment_list,
                              ordered_consensus_chain_alignment_list,
                              ordered_consensus_TAAIGScore_alignment_list,
                              template_sequence_alignment_string,
                              consensus_alignment_file="consensus_alignment",
                              chars_per_line=162,
                              iteration=1):
    """
    ...
    with open(consensus_alignment_file+".iteration"+str(iteration), 'w') as f:
        total_chars_per_line = len(template_sequence_alignment_string)
        start = 0
        while (start+chars_per_line-1) <= total_chars_per_line:
            print("\n\n")
            print(template_sequence_alignment_string[start:(start+chars_per_line-1)].replace(' ', ''))
            f.write(template_sequence_alignment_string[start:(start+chars_per_line-1)] + "\n")
            for peptide_line, chain_alignment_list, TAAIGScoreTuple_list in zip(ordered_consensus_peptide_alignment_list,
                                                                                ordered_consensus_chain_alignment_list,
                                                                                ordered_consensus_TAAIGScore_alignment_list):
                #print peptide_line[start:(start+chars_per_line-1)]
                #f.write(peptide_line[start:(start+chars_per_line-1)] + "\n")
                ## RECOVER THE CHAIN ALIGNMENT STRING
                chain_line = '     '
                for word in chain_alignment_list:
                    if word == '-':
                        ...
                        chain_line += "%12s" % word
                print(''.join(chain_line[start:(start+chars_per_line-1)]).replace(' ', ''))
                f.write(''.join(chain_line[start:(start+chars_per_line-1)]) + "\n")
                #print TAAIGScoreTuple_list[start:(start+chars_per_line-1)])
                #f.write(TAAIGScoreTuple_list[start:(start+chars_per_line-1)] + "\n")
            start += chars_per_line-1
        print("\n\n")
        f.write("\n\n\n")
        print(template_sequence_alignment_string[start:].replace(' ', ''))
        ...
        for peptide_line, chain_alignment_list, TAAIGScoreTuple_list in zip(ordered_consensus_peptide_alignment_list,
            ...
            for word in chain_alignment_list:
                if word == '-':
                    ...
                    chain_line += "%12s" % word
            print(''.join(chain_line[start:]).replace(' ', ''))
...
def build_Alignment_Tree(reference_aligned_chain,
                         refchain_overlappingChainsList_dict,
                         ordered_consensus_protein_alignment_list,
                         ordered_consensus_chain_alignment_list):
    """
...
def check_if_subgroup(overlappingChainsGroup1_tuple, all_overlapped_chain_alignments_list):
    """
        FUNCTION that finds if a contig of overlapping chain alignments is a subgroup of another bigger contig.
    """
...
def save_overlappingChainsGroups(overlapped_chain_alignments_list, tuple_index_list, iterationNo_list):
    ...
    """
    print("DEBUG: overlapped_chain_alignments_list=", overlapped_chain_alignments_list)
...
def create_overlapped_chain_alignments(ordered_consensus_peptide_alignment_list,
                                       ordered_consensus_chain_alignment_list,
                                       OVERLAP_CUTOFF):
    """
    ...
    for row_index in range(len(ordered_consensus_chain_alignment_list)):    # iterate over all chain alignments
        ...
        if len(overlapping_chain_segment_list) < OVERLAP_CUTOFF:
            continue
        ...
        for query_aligned_chain in ordered_consensus_chain_alignment_list[row_index+1:]: # iterate over all remaining aligned chains to find overlaps
            ...
            try:
                if query_aligned_chain[start] == '-' \
                        and overlapping_chain_segment_list == query_aligned_chain[start+1:(start+1+len(overlapping_chain_segment_list))] \
                        and query_aligned_chain[start+1+len(overlapping_chain_segment_list)] != '-' \
                        and not query_aligned_chain[start+1+len(overlapping_chain_segment_list)] in consensus_sequence_list:
                ...
                if len(query_aligned_chain) == (start+1+len(overlapping_chain_segment_list)) \
                        and query_aligned_chain[start] == '-' \
                        and overlapping_chain_segment_list == query_aligned_chain[start+1:(start+1+len(overlapping_chain_segment_list))]:
                    overlappingChains_list.append(query_aligned_chain)                   # save the overlaping chain
                    #ordered_consensus_chain_alignment_list.remove(query_aligned_chain)  # and remove it from the ordered_consensus_chain_alignment_list
                    continue
    ...
    print("DEBUG: refchain_overlappingChainsList_dict=", refchain_overlappingChainsList_dict)
    print("DEBUG: ordered_consensus_peptide_alignment_list=", ordered_consensus_peptide_alignment_list)
    print("DEBUG: ordered_consensus_chain_alignment_list=", ordered_consensus_chain_alignment_list)
    for reference_aligned_chain in list(refchain_overlappingChainsList_dict.keys()):
        all_overlapped_chain_alignments_list.extend(build_Alignment_Tree(reference_aligned_chain,
                                                                         refchain_overlappingChainsList_dict,
                                                                         ordered_consensus_peptide_alignment_list,
                                                                         ordered_consensus_chain_alignment_list))
    ...
    assert overlappingChainsGroup_num > 0, \
        Debuginfo("ATTENTION: NO OVERLAPPING PEPTIDES WERE CREATED! PROBABLY THERE IS SOMETHING WRONG WITH YOUR DATA. "
                  "LOOK AT THE connectivities_all AND amino_acid_type_prediction_probabilities FILES.", fail=True)
...
def write_overlapped_chains_alignment(template_sequence_alignment_string,
                                      consensus_alignment_file="consensus_alignment",
                                      chars_per_line=162,
                                      iteration=1):
    """
    ...
    with open(consensus_alignment_file+".overlapped_chains.iteration"+str(iteration), 'w') as f:
        total_chars_per_line = len(template_sequence_alignment_string)
        ...
        for overlappingChains_tuple in unique_overlapped_chain_alignments_set:
            ...
            while (start+chars_per_line-1) <= total_chars_per_line:
                #print "\n\n"
                #print template_sequence_alignment_string[start:(start+chars_per_line-1)]
                f.write(template_sequence_alignment_string[start:(start+chars_per_line-1)] + "\n")
                for chain_alignment_tuple in overlappingChains_tuple:
                    ...
                    for word in chain_alignment_tuple:
                        if word == '-':
                            ...
                            chain_line += "%12s" % word
                    #print chain_line[start:(start+chars_per_line-1)]
                    f.write(chain_line[start:(start+chars_per_line-1)] + "\n")
...
def write_consensus_alignment_and_absolute_macthes(template_sequence_alignment_string,
    ...
    """
    print("DEBUG: write_consensus_alignment_and_absolute_macthes input args:",
          "template_sequence_alignment_string=",template_sequence_alignment_string,
          "refchain_overlappingChainsList_dict=",refchain_overlappingChainsList_dict,
          "max_peptide_length=",max_peptide_length,
          "ordered_consensus_peptide_alignment_list=",ordered_consensus_peptide_alignment_list,
          "ordered_consensus_chain_alignment_list=",ordered_consensus_chain_alignment_list,
          "ordered_consensus_TAAIGScore_alignment_list=",ordered_consensus_TAAIGScore_alignment_list)
    ...
    assert len(consensus_sequence_alignments_list) > 0, \
        Debuginfo("FAIL: consensus sequence alignment is empty! Relax your mratio,"
    ...
    with open(args.consensus_alignment_file+".overlapped_chains_common_sequence.iteration"+str(iteration), 'w') as f:
        ...
        for start, end in zip(list(range(0,len(absolute_matches_alignment),17)),
                              list(range(17,len(absolute_matches_alignment),17))):
    ...
    if args.USE_ALL_PEPTIDES_FOR_PREDICTION:    ## use all peptides to score the AAIG prediction
        ...
        for chain_alignment_list, TAAIGScoreTuple_list in zip(ordered_consensus_chain_alignment_list,
                                                              ordered_consensus_TAAIGScore_alignment_list):
    ...
    print("DEBUG: iteration %i ordered_consensus_chain_alignment_list=%i" % (iteration, len(ordered_consensus_chain_alignment_list)))
    print("DEBUG: iteration %i filtered_ordered_consensus_chain_alignment_list=%i" % (iteration, len(filtered_ordered_consensus_chain_alignment_list)))
    ...
    print("DEBUG: consensus_score_alignments_TAAIG_array=", consensus_score_alignments_TAAIG_array)
    with open(args.consensus_alignment_file+".prediction_scores.iteration"+str(iteration), 'w') as f:
        ...
        for start, end in zip(list(range(0,len(consensus_score_alignments_list[0]),5)),
            ...
            for row_index in range(len(consensus_score_alignments_list)):
                new_row = [re.sub(r"^\s0\.0", "", str(TAAIG)+" "+str(ConsScore)) for TAAIG, ConsScore in zip(consensus_score_alignments_TAAIG_array[row_index],
...
def filter_alignments_using_absolute_matches(absolute_matches_alignment,
                                             ordered_consensus_peptide_alignment_list,
                                             ordered_consensus_chain_alignment_list,
                                             ordered_consensus_TAAIGScore_alignment_list,
    ...
    """
    print("DEBUG filter_alignments_using_absolute_matches: ordered_consensus_chain_alignment_list=", ordered_consensus_chain_alignment_list)
    ...
    if SPECTRUM_COMBO == "TOCSY-HCNH" and 'N/A' in absolute_matches_alignment[0]:
        ...
        for position in range(len(chain_alignment_list)):
            try:
                if chain_alignment_list[position] == '-':
                    ...
                    continue
                elif (chain_alignment_list[position] not in absolute_matches_alignment) and absolute_matches_alignment[position] == '-':
                    continue
                    ...
                    #try:
                    print("DEBUG: found inagreement at position ", position,
                          ", deleting chain_alignment_list=", chain_alignment_list)
...
def print_results_summary(template_sequence_alignment_string,
    ...
    consensus_score_alignments_TAAIG_array = np.array(consensus_score_alignments_list, dtype=object)   # use dtype=object to be able to change the elements to every string length
    consensus_score_alignments_Score_array = np.zeros( (max_num_of_predictions, len(ordered_consensus_chain_alignment_list[0])) )
    for position in range(len(ordered_consensus_chain_alignment_list[0])):
        row_index = 0
        TAAIGConsScoreTuples_list = list(position_TAAIG_confidenceScore_mdict[position].items())
        if len(TAAIGConsScoreTuples_list) > 0:
    ...
    with open(out_fname, 'w') as f:
        ...
        if i_iminus1_dict != None:
            for i in range(len(consensus_absolute_matches_alignment)-1):
                ...
                if TIG_i != '-' and TIG_iplus1 != '-':
                    try:
                        connectivity = [c for c in i_iminus1_dict[TIG_iplus1] if c[0]==TIG_i][0]    # connectivity = (TIG_i, occupancy, TOCSY_resonnum, [intersection], [multi])
                        print("DEBUG: connectivity=", connectivity)
                        connectivities_alignment.append(str(connectivity[1])+"/"+str(connectivity[2]))
                    except (IndexError, KeyError):
                        connectivities_alignment.append('-')
        ...
        #print "DEBUG: template_sequence_alignment_string=", template_sequence_alignment_string
        for start, end in zip(list(range(0,len(consensus_absolute_matches_alignment),17)), list(range(17,len(consensus_absolute_matches_alignment),17))):
            # print "DEBUG: table=", table
            # print "DEBUG: headers=", headers
            headers = template_sequence_alignment_string[start:end]
